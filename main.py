...
import csv
import pandas as pd
from src.filter_transactions import search_transactions
...
def main():
    ...
    while True:
        ...
        if status in ['EXECUTED', 'CANCELED', 'PENDING']:
            filtered_transactions = [t for t in transactions if
                                     isinstance(t.get('state'), str) and t['state'].upper() == status]
    ...
    if filter_rub:
        filtered_transactions = [t for t in filtered_transactions if 'RUB' in (
        t.get('operationAmount', {}).get('currency', {}).get('name', ''), t.get('currency_name', ''))]
